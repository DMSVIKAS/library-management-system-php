...
if(isset($_POST['submit'])) {
    ...
    } elseif(empty($firstname) || empty($lastname) || empty($username) || empty($password) || empty($roll) || empty($email) || empty($contact)) {
        $error = "Please fill all fields";
    } else {
        $stmt = $db->prepare("INSERT INTO `student`(`firstname`, `lastname`, `username`, `password`, `roll`, `email`, `contact`) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssiss", $firstname, $lastname, $username, $password, $roll, $email, $contact);
        if($stmt->execute()) {
